...
- patchMiscLineDom() does equivalent live patching for misc lines (unit price input, amt-cell-{id}, misc-edit-unitprice-{id}, lab tag)
...
- Misc items: component breakdown from MPL picker (search/filter, supplier price +10% markup, auto-appends description) or manual free-text (also +10%, auto-appends); mat cost = read-only sum of components
