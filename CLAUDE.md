...
**Sandbox build (in progress):** 2nd project `firstlight-sandbox`. User is creating it + running the generated CREATE TABLE SQL in ITS SQL editor, then will hand over the sandbox **Project URL + anon public key** (anon key is safe to share — public by design; never the service_role key or DB password). Sandbox schema decisions: **RLS OFF, NO foreign keys** (deliberate — keeps the restore test clean; RLS added later as its own step). `id` columns are `bigint generated by default as identity primary key` (auto-generate but still accept the backup's id values). Single-column PKs: quotes.ref, deliverables.code, mpl.code; the rest keyed on `id`.

**Restore (Phase 2) — to build next:** import a backup JSON and write rows back per table. Two modes — **Merge** (upsert, safe default) vs **Wipe & replace** (destructive, behind a strong typed confirm); auto-backup before any restore; a **dry-run** mode (reports per-table counts, writes nothing) as the first safety layer. Test ONLY against sandbox/local — never live writes. Acceptance = 6-point proof: (1) backup from live, (2) restore into empty sandbox, (3) row counts match, (4) spot-check relationships (quote→lines, deliverable→materials), (5) load the app pointed at the sandbox and confirm quotes render, (6) wipe sandbox + restore again.
